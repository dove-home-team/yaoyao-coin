...
public class CoinCommand {
    ...
    private static int setSlotVisibility(CommandSourceStack source, Collection<? extends Entity> targets, CoinType type, boolean visible) {
        for(Entity target : targets) {
            if(target instanceof ServerPlayer player) {
                LazyOptional<CoinInventoryCapability.CoinInventory> inv = player.getCapability(CoinCapability.COIN_INVENTORY).cast();
                inv.ifPresent(coin -> {
    ...
    private static int giveCoin(CommandSourceStack source, Collection<? extends Entity> targets, CoinType type, int number) {
        for(Entity target : targets) {
            if(target instanceof ServerPlayer player) {
                LazyOptional<CoinInventoryCapability.CoinInventory> inv = player.getCapability(CoinCapability.COIN_INVENTORY).cast();
                inv.ifPresent(coin -> {
    ...
    private static int consumeCoin(CommandSourceStack source, Collection<? extends Entity> targets, CoinType type, int number) {
        for(Entity target : targets) {
            if(target instanceof ServerPlayer player) {
                LazyOptional<CoinInventoryCapability.CoinInventory> inv = player.getCapability(CoinCapability.COIN_INVENTORY).cast();
                inv.ifPresent(coin -> {
